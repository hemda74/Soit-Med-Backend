    ...
    public class LegacyDataSyncService : ILegacyDataSyncService
    {
        ...
        public async Task<CustomerMachinesSyncDto?> GetCustomerMachinesFromTbsAsync(int legacyCustomerId)
        {
            try
            {
                ...
                // Get customer info
                var customer = await tbsContext.StkCustomers.AsNoTracking()
                ...
                _logger.LogInformation("Getting machines for customer {CustomerId} ({CustomerName}) from TBS", 
                    legacyCustomerId, customer.CusName);

                // Use the SAME logic as MediaApi ContractService.GetMachinesByCustomerIdAsync
                var machines = await GetMachinesForCustomerAsync(tbsContext, legacyCustomerId);

                // Get visits for each machine
        ...
        private async Task<Client?> CreateClientFromTbsAsync(int legacyCustomerId, CustomerMachinesSyncDto customerData)
        {
            try
            {
                ...
                var client = new Client
                {
                    ...
                    // IsActive is a method, not a property - removed initialization
                };
        ...
        private static string GetVisitTypeName(int visitingTypeId)
        {
            return visitingTypeId switch
            {
                1 => "دورية (Periodic)",
                2 => "طارئة (Repair)",
                3 => "تركيب (Installation)",
                4 => "فحص (Inspection)",
                _ => $"نوع {visitingTypeId} (Type {visitingTypeId})"
            };
        }

        private static string GetVisitStatusName(int statusId)
        {
            return statusId switch
            {
                1 => "مجدولة (Scheduled)",
                2 => "تم التنفيذ (Completed)",
                3 => "قيد المراجعة (Pending Review)",
                4 => "مغلقة (Closed)",
                _ => "غير معروف (Unknown)"
            };
        }

        private static List<string> ParseMediaFiles(string? visitFiles, string? reportFiles)
        {
            var files = new List<string>();

            if (!string.IsNullOrWhiteSpace(visitFiles))
            {
                files.AddRange(visitFiles.Split(new[] { '|', ',', ';' }, StringSplitOptions.RemoveEmptyEntries)
                    .Select(f => f.Trim())
                    .Where(f => !string.IsNullOrWhiteSpace(f)));
            }

            if (!string.IsNullOrWhiteSpace(reportFiles))
            {
                files.AddRange(reportFiles.Split(new[] { '|', ',', ';' }, StringSplitOptions.RemoveEmptyEntries)
                    .Select(f => f.Trim())
                    .Where(f => !string.IsNullOrWhiteSpace(f)));
            }
