...
-- =====================================================
-- This script seeds data without relying on AspNetUsers table

PRINT '=====================================================';
PRINT 'FINAL DATA SEEDING SCRIPT';
PRINT '=====================================================';

-- =====================================================
-- Step 1: Declare User ID variable
-- =====================================================
DECLARE @UserId NVARCHAR(450);

PRINT 'Checking for user tables...';
...
BEGIN
    ...
    -- Try to get ahmed@soitmed.com
    SET @UserId = (SELECT Id FROM AspNetUsers WHERE Email = 'ahmed@soitmed.com');
    
    IF @UserId IS NOT NULL
    BEGIN
        PRINT 'Found User ID for ahmed@soitmed.com: ' + @UserId;
    END
    ELSE
    BEGIN
        PRINT 'User ahmed@soitmed.com not found. Using first available user.';
...
BEGIN
    ...
    -- Check for other user tables
    IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Users]') AND type in (N'U'))
    BEGIN
        PRINT '✓ Users table exists';
        SET @UserId = (SELECT TOP 1 Id FROM Users);
        PRINT 'Using User ID from Users table: ' + @UserId;
    END
    ELSE
    BEGIN
        PRINT '✗ No user tables found. Using placeholder user ID.';
        SET @UserId = 'placeholder-user-id';
        PRINT 'Using placeholder User ID: ' + @UserId;
    END
END
...
-- Check if sales reports already exist
...
ELSE
BEGIN
    PRINT '✓ Sales reports already exist for this user.';
END
...
PRINT 'You can now test the sales module APIs.';
